        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div class="bg-white shadow-sm sm:rounded-lg p-6">
                    ...
                    <div class="mt-1 text-2xl font-semibold text-gray-900">{{ $stats['servicios_activos'] }}</div>
                </div>
                <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                ...
                @if ($proximas->isEmpty())
                    <p class="px-6 pb-6 text-sm text-gray-500">No tenés cuotas ni cargos pendientes. 🎉</p>
                @else
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Compra/Servicio</th>
                                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Detalle</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($proximas as $item)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $item['vencimiento']->format('d/m/Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <a href="{{ $item['url'] }}" class="text-indigo-600 hover:text-indigo-900">{{ $item['descripcion'] }}</a>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{{ $item['detalle'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">$ {{ number_format($item['monto'], 2, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
